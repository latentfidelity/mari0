...
1. Thread report-only player render-intent direction-scale source details into `iw2wth_runtime::shell` and harness/CLI summaries, preserving the legacy player `pointingangle` draw-scale decision separately from animation facing without live rendering or player mutation.
