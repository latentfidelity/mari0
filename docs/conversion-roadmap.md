...
- Report-only player render-intent hat draw previews from the one-player shell state, including default hat slots, small/big hat image/config offsets, standard-hat tinting, stacked hat origins, legacy post-`graphic[1..3]` and pre-`graphic[0]` ordering metadata, harness/CLI detail exposure, and no live rendering or player mutation.
- Report-only player render-intent direction-scale source previews from the one-player shell state, including the legacy `pointingangle > 0` draw-scale decision preserved separately from animation facing, harness/CLI detail exposure, and no live rendering or player mutation.
- Report-only player render-intent portal-clone/scissor previews from the one-player shell state, including legacy entry/exit portal scissor rectangles, non-live `portalcoords` duplicate draw body/rotation/animation-direction output, portal-flipped draw scale, harness/CLI detail exposure, and no live rendering or player mutation.
- Report-only portal aiming render-intent previews from the one-player shell state, including legacy portal trace source/target, dot timer constants, first dot sample geometry, crosshair rotation/color/image metadata, harness/CLI detail exposure, and no live rendering or portal mutation.
- Report-only portal projectile render-intent previews from adapter-provided portal projectile snapshots, including legacy particle-before-head draw order, head/particle image paths, color/origin/draw coordinates, harness/CLI detail exposure, and no live rendering, projectile physics migration, or portal mutation.
- Report-only emancipation grill render-intent previews from adapter-provided grill snapshots, including horizontal/vertical scissor rectangles, line color/rectangle metadata, particle direction/origin/rotation formulas, side image draws, harness/CLI detail exposure, and no live rendering or grill physics migration.
- Report-only door render-intent previews from adapter-provided door snapshots, including horizontal/vertical piece and center image paths, timer-driven y-offset and center rotation formulas, draw coordinates, origins, harness/CLI detail exposure, and no live rendering or door physics/entity migration.
- Report-only wall indicator render-intent previews from adapter-provided indicator snapshots, including lighted/off quad selection, source rectangles, white draw color, image path, draw coordinates, harness/CLI detail exposure, and no live rendering or wall-indicator physics/entity migration.

Next queue:

1. Thread report-only wall timer render-intent details into `iw2wth_runtime::shell` and harness/CLI summaries, preserving the Lua draw-loop timer quad/image metadata without live rendering or wall-timer update/entity migration.
...
- `iw2wth_runtime::shell`, `iw2wth_runtime::harness`, and `iw2wth-runtime-harness` expose report-only player render-intent previews from the one-player shell state, preserving body, animation-facing metadata, pointing-angle direction-scale source metadata, portal-clone/scissor metadata, render-frame selection, size/power-up, ducking, fire-animation metadata, atlas quad/source-rectangle metadata, color-layer image draw order, hat draw metadata, structured CLI detail, and no live rendering or player mutation.
- `iw2wth_runtime::shell`, `iw2wth_runtime::harness`, and `iw2wth-runtime-harness` expose report-only portal aiming render-intent previews from the one-player shell state, preserving Lua draw-loop trace source/target metadata, dot sample geometry, crosshair rotation/color/image details, structured CLI detail, and no live rendering or portal mutation.
- `iw2wth_runtime::shell`, `iw2wth_runtime::harness`, and `iw2wth-runtime-harness` expose report-only portal projectile render-intent previews from adapter-provided projectile snapshots, preserving Lua particle-before-head draw order, active-head timer gating, image paths, draw coordinates, color/origin metadata, structured CLI detail, and no live rendering, projectile physics, or portal mutation.
- `iw2wth_runtime::shell`, `iw2wth_runtime::harness`, and `iw2wth-runtime-harness` expose report-only emancipation grill render-intent previews from adapter-provided grill snapshots, preserving horizontal/vertical scissor rectangles, translucent line draw metadata, particle direction/origin/rotation formulas, side-image draw metadata, structured CLI detail, and no live rendering or grill physics migration.
- `iw2wth_runtime::shell`, `iw2wth_runtime::harness`, and `iw2wth-runtime-harness` expose report-only door render-intent previews from adapter-provided door snapshots, preserving horizontal/vertical piece and center image paths, Lua timer-driven `ymod` and center rotation formulas, draw coordinates, origins, structured CLI detail, and no live rendering, door physics, or door entity mutation.
- `iw2wth_runtime::shell`, `iw2wth_runtime::harness`, and `iw2wth-runtime-harness` expose report-only wall indicator render-intent previews from adapter-provided indicator snapshots, preserving Lua lighted/off quad selection, source rectangles, image path, white draw color, draw coordinates, structured CLI detail, and no live rendering, wall-indicator physics, or wall-indicator entity mutation.
